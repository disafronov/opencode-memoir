...
import { coercePaths, MEMOIR_GET_MAX_KEYS, tryPrettyJson, SECRET_PATTERN } from './utils.js';
...
const memoirRecall = tool({
  ...
  execute: async (args: MemoirRecallArgs) => {
    ...
    for (const namespace of namespaces) {
      const summarizeResult = await runMemoir(['--json', '-s', store, 'summarize', '--depth', '3', '-n', namespace], { cwd: store });
...
const memoirGet = tool({
  ...
  execute: async (args: MemoirGetArgs) => {
    ...
    const getResult = await runMemoir(['--json', '-s', store, 'get', ...keys, '-n', args.namespace ?? 'default'], { cwd: store });
