...
import { clearSession, getPendingEdits, getToolMetrics } from "../src/capture.ts";
...
describe("chat.message hook", () => {
  ...
  it("triggers recall gate for code-related messages", async () => {
    await hooks["chat.message"](
      ...
      {
        parts: [
          { type: "text", text: "Can you implement the caching layer for the payment module?" },
        ],
      } as never,
    );
  ...
  it("does not crash on empty parts", async () => {
    await hooks["chat.message"]({ sessionID: SID } as never, { parts: [] } as never);
    assert.ok(!pendingRecall.has(SID));
  });
...
describe("system.transform hook", () => {
  ...
  it("does not throw on missing sessionID", async () => {
    await hooks["experimental.chat.system.transform"]({} as never, { system: [] } as never);
    assert.ok(true);
  });
});
