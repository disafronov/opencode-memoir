...
import { setStoreTestOverrides, getCurrentBranch, codeGitBranch, branchExistsInMemoir, autoMatchMemoirBranch, readMemoirValue } from '../src/store.ts';
...
describe('getCurrentBranch', () => {
  ...
  it('returns unknown on CLI error', async () => {
    setStoreTestOverrides({
      runMemoirFn: async () => { throw new Error('fail'); },
    });
    const result = await getCurrentBranch(TEST_STORE);
    assert.strictEqual(result, 'unknown');
  });
...
describe('codeGitBranch', () => {
  ...
  it('returns empty string on git error', async () => {
    setStoreTestOverrides({
      execFileAsyncFn: async () => { throw new Error('fail'); },
    });
    const result = await codeGitBranch();
    assert.strictEqual(result, '');
  });
...
describe('branchExistsInMemoir', () => {
  ...
  it('returns false for empty name', async () => {
    ...
    setStoreTestOverrides({
      runMemoirFn: async () => { throw new Error('should not be called'); },
    });
